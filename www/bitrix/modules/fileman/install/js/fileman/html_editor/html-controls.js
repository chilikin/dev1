...
*/
	...
	function VideoDialog(editor, params)
	{
		...
		this.YOUTUBE_RE = new RegExp('(youtube.com)|(youtu.be)', 'ig');
		this.VIMEO_RE = new RegExp('vimeo.com', 'ig');
		this.RUTUBE_RE = new RegExp('vimeo.com', 'ig');
	...
	VideoDialog.prototype.Build = function()
	{
		...
		this.pSource = r.rightCell.appendChild(BX.create('INPUT', {props: {id: this.id + '-source', type: 'text', className: 'bxhtmled-90-input'}}));
		...
		// Size
		r = this.AddTableRow(pTableWrap, {label: BX.message('BXEdVideoSize') + ':', id: this.id + '-size'});
		this.pSize = r.rightCell.appendChild(BX.create('SELECT', {props: {id: this.id + '-size'}}));
		for (var i = 0; i < this.sizes.length; i++)
		{
			this.pSize.options.add(new Option(this.sizes[i].width + ' x ' + this.sizes[i].height, i, false, false));
		}
		BX.addClass(r.row, 'bxhtmled-video-ext-row');

//		this.pSize.options.add(new Option(BX.message('BXEdVideoSizeCustom'), '', false, false));
//		r.rightCell.appendChild(BX.create('LABEL', {props: {className: 'bxhtmled-width-lbl'}, text: BX.message('BXEdImgWidth') + ': ', attrs: {'for': this.id + '-width'}}));
//		this.pWidth = r.rightCell.appendChild(BX.create('INPUT', {props: {id: this.id + '-width', type: 'text'}}));
//		this.pHeight = r.rightCell.appendChild(BX.create('INPUT', {props: {id: this.id + '-height', type: 'text'}}));

		// Preview
		r = this.AddTableRow(pTableWrap, {label: BX.message('BXEdVideoPreview') + ':', id: this.id + '-preview'});
		//
		BX.addClass(r.row, 'bxhtmled-video-ext-row');
	...
	{
		var value = this.pSource.value;

		if (value !== this.lastSourceValue)
		{
			this.lastSourceValue = value;
			// 1. Youtube, vimeo, rutube
	...
	VideoDialog.prototype.Show = function(nodes, savedRange)
	{
		//this.SetValues(values);
