...
abstract class DataManager
{
	...
	public static function add(array $data)
	{
		...
		$event = new Event($entity, self::EVENT_ON_ADD, array("fields"=>$data), true);
		$event->send();
		...
		foreach ($data as $fieldName => $value)
		{
			...
			$data[$fieldName] = $field->modifyValueBeforeSave($value);
		}
	...
	public static function update($primary, array $data)
	{
		...
		$event = new Event($entity, self::EVENT_ON_UPDATE, array("id"=>$primary, "primary"=>$primary, "fields"=>$data), true);
		$event->send();
