...
class CTextParser
{
	...
	function convertText($text)
	{
		...
		foreach ($this->allow as $tag => $val)
		{
			...
			switch ($tag)
			{
				case "CODE":
					$text = preg_replace_callback(
						"/\\[code[^\\]]*\\](.*?)\\[\\/code[^\\]]*\\]/is".BX_UTF_PCRE_MODIFIER,
	...
	public function preconvertCode($matches)
	{
		return '[code]'.$this->pre_convert_code_tag($matches[2]).'[/code]';
	}
	...
	{
		return $this->convert_code_tag($matches[1]);
	}
	...
	{
		return $this->pre_convert_anchor_tag($matches[1], $matches[2]);
	}
