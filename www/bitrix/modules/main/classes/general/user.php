...
abstract class CAllUser extends CDBResult
{
	...
	function Login($login, $password, $remember="N", $password_original="Y")
	{
		...
		if($user_id>0)
		{
			...
			$APPLICATION->set_cookie("LOGIN", $this->GetParam("LOGIN_COOKIES"), time()+60*60*24*30*60, '/', false, false, COption::GetOptionString("main", "auth_multisite", "N")=="Y");
		}
	...
	function Delete($ID)
	{
		...
		$res = $DB->Query("DELETE FROM b_user WHERE ID=".$ID." AND ID<>1", true);
